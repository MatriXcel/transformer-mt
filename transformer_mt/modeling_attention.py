...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        if kv == None:
            k = self.k(q) #[batch_size, source_seq_len, hidden_dim] 
            q = self.q(q) #[batch_size, source_seq_len, hidden_dim] 
            v = self.v(q) #[batch_size, source_seq_len, hidden_dim] 
        else:
            k = self.k(kv) #[batch_size, target_seq_len, hidden_dim] 
            q = self.q(q)  #[batch_size, target_seq_len, hidden_dim] 
            v = self.v(kv) #[batch_size, target_seq_len, hidden_dim] 
        ...
        if key_padding_mask is not None:
           ...
           # key_padding_mask = key_padding_mask.to(scores.device)
            ...
            key_padding_mask = torch.repeat_interleave(key_padding_mask, torch.LongTensor([attending_seq * self.num_heads] * bs, device=key_padding_mask.device), dim=0) \
                .reshape((bs * self.num_heads, attending_seq, key_padding_mask.shape[1]))

            scores.masked_fill_(key_padding_mask.bool(), float("-inf"), device=scores.device)
