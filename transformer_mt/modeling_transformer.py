...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.cross_attention = MultiHeadAttention(hidden, hidden, num_heads, causal=False)
        
        self.att_layer_norm = torch.nn.LayerNorm(hidden)
        self.cross_att_layer_norm = torch.nn.LayerNorm(hidden)
...
#self, q, kv=None, key_padding_mask=None, return_attention=False
        h2 = self.cross_attention(h, encoder_hidden_states, key_padding_mask=key_padding_mask) #[batch_size, seq_len, hidden]
        h2 = h2 + h

        h2 = self.cross_att_layer_norm(h2)

        h3 = self.fcn(h2)    #[batch_size, seq_len, hidden]
        h3 = self.dropout(h3)

        h3 = h3 + h2        
        
        h3 = self.fcn_layer_norm(h3)    #[batch_size, seq_len, hidden]
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_embeddings = nn.Embedding(self.src_vocab_size, hidden)
        self.decoder_embeddings = nn.Embedding(self.tgt_vocab_size, hidden)
        self.positional_emb = nn.Embedding(self.tgt_vocab_size, hidden)

        self.out_proj = nn.Linear(hidden, self.tgt_vocab_size)
        ...
        self.encoder_layers = nn.ModuleList([ TransformerEncoderLayer(hidden, num_heads, fcn_hidden, dropout) for i in range(num_layers)])
        self.decoder_layers = nn.ModuleList([ TransformerDecoderLayer(hidden, num_heads, fcn_hidden, dropout) for i in range(num_layers)])
        # YOUR CODE ENDS HERE
    ...
    def forward(
        ...
        logits = self._decode(encoder_hidden_states, decoder_input_ids, key_padding_mask=key_padding_mask)
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        src_embeds = self.encoder_embeddings(input_ids)
        src_embeds_with_pos = self._add_positions(src_embeds)
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        tgt_embeds = self.decoder_embeddings(decoder_input_ids)
        tgt_embeds_with_pos = self._add_positions(tgt_embeds)

        decoder_hidden_states = tgt_embeds_with_pos
        
        for i, l in enumerate(self.decoder_layers):
            decoder_hidden_states = l(tgt_embeds_with_pos, encoder_hidden_states, key_padding_mask=key_padding_mask) #[batch_size, seq_len, hidden]
